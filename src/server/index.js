...
if (process.env.MASTER_INSTANCE === 'true') {
  ...
  (async () => {
    ...
    const gentoken = promisify(crypto.randomBytes);
    const INSTANCETOKEN = await gentoken(64).then(buf => buf.toString('base64url'));
    const smToken = await gentoken(16).then(buf => buf.toString('base64url'));
    ...
    const SummonChild = (child_Id = child_index) => {
      ...
      let commands = [
        ...
        `docker-machine create \
          ...
          ${smToken}-sdc-factory-about-child${child_Id}`,

        //Loads into the machine's environment
        `eval $(docker-machine env ${smToken}-sdc-factory-about-child${child_Id})`,

        //builds the image
        `docker build -f Child.Dockerfile -t sdc-about-child`,

        //starts a container
        `docker run -d --publish 80:3002 --env-file .env sdc-about-child`,

        //unsets the machine's environment
...
if (process.env.NODE_ENV !== 'test') {
  server.listen(PORT, () => {
    if (process.env.MASTER_INSTANCE === 'true') {
      ...
      axios.post(`/${process.env.INSTANCETOKEN}`);
    }
